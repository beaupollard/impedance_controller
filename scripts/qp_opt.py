...
class qp_opt():
    def __init__(self,sim,F_des=np.zeros(6),optimize=True,hybrid=False,ee_site='mep:ee',target_site='wpt1',qvel_index=np.array([0,1,2,3,4,5]),peg_name=['peg','peg_base'],kp=np.array([100,120,120,120,120,120]),kd=np.array([40,60,60,60,60,60])):
        ...
        self.ee_site_body=self.sim.model.site_bodyid[self.sim.model.site_name2id(self.ee_site)]
  
    def run_opt(self, ext_force):
        ...
        tau = self.force_control(hybrid=self.hybrid,F_des=self.F_des, ext_force=ext_force)
    ...
    def force_control(self,hybrid=False,F_des=np.zeros(6), ext_force=np.zeros(6)):
        buffer=50
        F_ext=self.endeffector_force()

        self.apply_ee_force(ext_force)
    ...
    def apply_ee_force(self,ext_force):
        R=self.sim.data.get_site_xmat(self.ee_site)
        Fext=-np.concatenate((np.linalg.inv(R)@ext_force[:3],np.linalg.inv(R)@ext_force[3:]))
        self.sim.data.xfrc_applied[self.ee_site_body]=Fext